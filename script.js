...
const UIController = ((DOMStrings,AnimationClass,getSongs)=>{
        ...
        swipeDown.get('swipe').set({direction : Hammer.DIRECTION_VERTICAL});
        ...
        // Add Song UI 
           ...
           for(let i in getSongs.newSong){
                
                DOM.audioHorListParent.innerHTML += `<div class="music-hor__item">
                                                    <div class="music-hor__item-inner">
                                                        <img src="img/${getSongs.newSong[i].image}" class="music-hor__img">
                                                        <h3 class="music-artist-name">${getSongs.newSong[i].name}</h3>
                                                    </div>
                                                    <div class="music-hor__item-inner"> 
                                                        <p class="music-hor__music-duration">${getSongs.newSong[i].duration}</p>                  
                                                    </div>
                                                </div> `;
...
const musicController = ((DOMStrings,getSongs) => {
    ...
    musicHoriList.forEach( (item, index) => {
        item.addEventListener('click',()=>{
            currenListObject = getSongs.newSong;
            updateAudio(index);    
        });
    });
    ...
    const getRandomSong = () => {
          return Math.floor(Math.random() * (currenListObject.length - 1) )+ 1;
    };


    audioCtrl.nextBtn.forEach( btn =>{
        btn.addEventListener('click',() =>{
            if( currenPlayingSongIndex === getSongs.songs.length-1)
    ...
    audioCtrl.prevBtn.forEach( btn =>{
        btn.addEventListener('click',() =>{
            if( currenPlayingSongIndex === 0)
                currenPlayingSongIndex = getSongs.songs.length;
            updateAudio(--currenPlayingSongIndex);
        });
    });
